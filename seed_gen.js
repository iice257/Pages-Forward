...
function generateBooksData() {
  const books = [];
  for (let i = 0; i < 50; i++) { // Generate 50 books
...
const books = generateBooksData();

let sql = "TRUNCATE TABLE books CASCADE;\n";

books.forEach(book => {
  const price = book.length === 'Short' ? 5000 : 10000;
  const stock = 5;
  ...
  const subtitle = book.subtitle ? `'${book.subtitle.replace(/'/g, "''")}'` : 'NULL';
  const description = book.description ? `'${book.description.replace(/'/g, "''")}'` : 'NULL';
  const cover = book.coverImage ? `'${book.coverImage}'` : 'NULL';
  // Fix check constraint for Non-fiction
  const type = book.tags.type === 'Non-Fiction' ? 'Non-fiction' : book.tags.type;
  const category = book.tags.category;
  const niche = book.tags.niche;
  const authorPop = book.popularity.author.toFixed(2);
  const bookPop = book.popularity.book.toFixed(2);
  ...
  sql += `INSERT INTO books (
    ...
    '${title}', '${slug}', '${author}', ${subtitle}, ${description}, ${cover}, 
    ${price}, ${stock}, '${book.status}', '${book.length}',
    '${type}', '${category}', '${niche}',
    ${authorPop}, ${bookPop}
  );\n`;
});

fs.writeFileSync(path.join(__dirname, 'supabase', 'migrations', '02_seed_data.sql'), sql);
